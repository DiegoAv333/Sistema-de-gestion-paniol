    // src/components/Modals/EditTeacherModal.jsx
    import { useState } from "react";
    import { useStore } from "../../context/StoreProvider";

    export default function EditTeacherModal({ teacher, onClose }) {
    const { updateTeacher } = useStore();
    ...
    const onSubmit = (e) => {
        e.preventDefault();
        try {
        updateTeacher(teacher.id, { name, department, email });
        onClose();
        } catch (err) {
        ...
        <div className="relative top-20 mx-auto p-5 border w-full max-w-md shadow-lg rounded-md bg-white">
            ...
            <div>
                ...
                {["","Construcción","Carpintería","Electricidad","Mecánica","Administración","Informática","Soldadura","Plomería"].map(d=>(
                    <option key={d} value={d}>{d || "Seleccionar…"}</option>
                ))}
                </select>
            </div>
            <div>
